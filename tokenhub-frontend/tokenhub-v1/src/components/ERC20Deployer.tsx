...
function ERC20Deployer() {
  ...
  const handleDeploy = async () => {
    try {
      ...
      const res = await api.post("/deploy/erc20", {
        tokenName,
        tokenSymbol,
        initialSupply,
      });
      ...
      <div className="row justify-content-center">
        <div className="col-md-8 col-lg-6">
          <div className="card shadow rounded-4 p-4">
            <h3 className="mb-4">ERC20 Token Deployer</h3>
            ...
            {response && (
              <div className="alert alert-success mt-4 text-start" role="alert">
                <div className="mb-2">
                  <strong>Name:</strong>
                  <span className="ms-2">{response.tokenName}</span>
                </div>
                <div className="mb-2">
                  <strong>Symbol:</strong>
                  <span className="ms-2">{response.tokenSymbol}</span>
                </div>
                <div className="mb-2">
                  <strong>Contract Address:</strong>
                  <span className="ms-2 text-break">{response.address}</span>
                </div>
                <div className="mb-0">
                  <strong>Total Supply:</strong>
                  <span className="ms-2">{response.totalSupply}</span>
                </div>
              </div>
            )}
